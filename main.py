...
# 目標網址
TARGET_URL = "https://www.ptt.cc/bbs/"  # 大寫為常數!!
...
def parser_article_content(url_list):
    ...
    for url_ in url_list:
        if url_ != None:
            ...
            page_html_code = BeautifulSoup(page_data.content, features="html.parser")  #從colab轉vscode必須加這個，否則只會回傳字串
            try:
...
def send_line_notify(msg="傳輸訊息"):
    ...
    res = requests.post(line_notify_url, headers=line_notify_header,
                        data=line_notify_body)
